...
To help in generating Wasm 3.0 code, we have designed and implemented a small DSL embedded in Haskell. The DSL is a small deeply embedded imperative language \cite{dsl} that abstracts away the explicit stack manipulation that is necessary when writing Wasm code directly. The DSL is close to Wasm 3.0 in its structure. A module in Wasm corresponds to a \verb|RtSpec| in the DSL, and contains the type definitions, imports, and functions of the module:
...
\end{minted}
The type definitions consist of a name and what type (can be a struct or an array) to bind it to:
\begin{minted}{haskell}
...
data RtExpr
    ...
    | IntLit Int
\end{minted}
Overall the DSL closely resembles Wasm 3.0

Using the constructs provided by the DSL, we can implement functions in a way that is close to how they would be implemented in Wasm 3.0, but in and easier-to-read and write manner. For example we can implement a function that calculates the length of the left spine of a node in a graph as follows:
\begin{minted}{haskell}
...
leftSpineLength = RtFunc
    ...
    , funcBody = Block
        ...
        -- traverseLeft is a shorthand for a while loop that traverses
        -- the left spine of a node until it reaches a leaf and executes
        -- the given body on each node visited
...
\end{minted}
In this example we make use of several constructs provided by the dsl 

On top of the DSL, we have made shorthand functions that generate a block of DSL expressions for patterns used in the original DSL used by Anand~\cite{anandthesis} for implementing reduction rules for the combinators and primitives used in MicroHs. This allows us to implement the runtime system entirely in the new DSL while mostly reusing the look and feel of the existing combinator and primitive implementations. As an example we first show the implementation of the reduction of the \verb|S| ($\text{S} x y z \rightarrow x z (yz)$) combinator in the original DSL:
\begin{minted}{haskell}
redRuleS :: [MixedInstr]
...
\end{minted}
As can be seen the implementation is quite verbose due to the explicit constructors used to build the graph patterns. Using the new DSL and the shorthand functions we can implement the same reduction rule as follows:
\begin{minted}{haskell}
redRuleS :: RtExpr
redRuleS = redRule n ln rn
    where
        ...
        rn = mkNode y z
\end{minted}
As can be seen the implementation makes use of the \verb|mkNode| function, this function generates a block of DSL expressions that constructs a node with the given left and right children. The new implementation is much more concise and easier to read, while still being close to the original implementation.
...
Notably the program does not make use of the Prelude, instead opting to define its own data types and functions, and making use of the primitive \verb|Int| type defined in MicroHs's \verb|Primitives| module. The results of the sanity check can be seen in Table~\ref{tab:sanity_check_results}.
\begin{table}[h]
    \centering
